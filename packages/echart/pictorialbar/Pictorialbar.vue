...
export default {
  ...
  methods: {
    updateChart() {
      ...
      const symbol = validatenull(this.option.symbol)
        ? spirit
        : 'image://' + this.option.symbol
      const color = this.option.color || '#fff'
      const fontSize = this.option.fontSize || 20
      let datas = optionData.series[0].data
      datas =
        typeof datas[0] !== 'object' ? datas : datas.map(item => item.value)
      const maxData = Math.max.apply(null, datas)
      let option = {
        title: this.title(),
